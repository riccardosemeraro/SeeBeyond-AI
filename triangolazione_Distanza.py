...
class calculate_distance():
    def __init__(self):
        ...
        angle_height = 60  # 63
        frame_rate = 7
        self.camera_separation = 9 #misura in pollici
        ...
        # variables
        self.maxsd = 2  # maximum size difference of targets, percent of frame
        self.klen = 3  # length of target queues, positive target frames required to reset set X,Y,Z,D

        # target queues
        self.x1k, self.y1k, self.x2k, self.y2k = [], [], [], []
        self.x1m, self.y1m, self.x2m, self.y2m = 0, 0, 0, 0

        # last positive target
        # from camera baseline midpoint
        self.X, self.Y, self.Z, self.D = 0, 0, 0, 0

    def triangolazione(self):

        #initialize variables
        ct1 = self.ct1
        ct2 = self.ct2
        targeter1 = self.targeter1
        targeter2 = self.targeter2
        angler = self.angler
        x1k = self.x1k
        y1k = self.y1k
        x2k = self.x2k
        ...
        #conn_sub_server(("192.168.0.103",15000),frame2)
        ...
        if x1k:
            ...
            return x2m, y2m, D, frame1, frame2
        else:
            return 0, 0, 0, frame1, frame2
...
class Frame_Motion:
    ...
    target_on_contour = False  # else use box size
    ...
    def targets(self, frame):
        ...
        # get contours
        contours, hierarchy = cv2.findContours(frame3, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE) #precedentemente era frame3, contours, hierarchy = cv2.findContours(frame3, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
class Frame_Angles:
    ...
    pixel_width = 640 #grandezza finestre
    pixel_height = 480
