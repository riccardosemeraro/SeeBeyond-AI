...
'''class Frame_Transfer:
    ...
    def in_frame(self, frame, x, y, D):
        #frame = cv2.resize(frame,(480,320))
        byte_frame = pickle.dumps(frame) #codifica in byte il frame
        self.s.sendall(byte_frame) #invia frame codificato in byte al server
    ...
    def conn_sub_server(self, indirizzo_server):
        try:
            self.s = socket.socket() #creazione del socket
            self.s.connect(indirizzo_server) #connessione al server
            print(f"Connession al Server: {indirizzo_server} stabilita")
